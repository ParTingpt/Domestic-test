(function(){
  ...
  const fallbackEmoji = {
    '阿拉斯加.png':'🐺','比格.png':'🐶','比熊.png':'☁️','边牧.png':'🧠','柴犬.png':'🍠','德牧.png':'🛡️','杜宾.png':'🐕‍🦺','哈士奇.png':'❄️','金毛.png':'🦮','柯基.png':'🦴','拉布拉多.png':'🤍','腊肠.png':'🛏️','马尔济斯.png':'🎀','萨摩耶.png':'☀️','松狮.png':'😶','泰迪.png':'✨'
  };
  ...
  style.textContent = `
    ...
    .hinticon.img-fallback,.avatar.img-fallback,.breed.img-fallback{font-size:22px!important}
    .hinticon img{width:36px;height:36px;image-rendering:pixelated;object-fit:contain;display:block}
    .avatar{font-size:0!important;overflow:hidden}
    .avatar img{width:88px;height:88px;image-rendering:pixelated;object-fit:contain;display:block}
  `;
  document.head.appendChild(style);

  function getEmoji(file){
    return fallbackEmoji[file] || '🐶';
  }

  function buildImg(file, size, cls){
    const src = ASSETS[file] || '';
    const alt = file.replace('.png','');
    return `<img class="${cls || ''}" src="${src}" alt="${alt}" style="width:${size}px;height:${size}px;image-rendering:pixelated;object-fit:contain">`;
  }

  function applyAsset(el, file, size){
    if (!el) return;
    const src = ASSETS[file];
    el.classList.remove('img-fallback');
    if (!src) {
      el.textContent = getEmoji(file);
      el.classList.add('img-fallback');
      return;
    }
    el.innerHTML = buildImg(file, size, 'dogti-pixel');
  ...
  function patchQuestionIcon(){
    ...
    applyAsset(icon, quizDogs[idx % quizDogs.length], 36);
  }
  ...
  function patchResultAvatar(){
    ...
    if (data && dogMap[data.dog]) applyAsset(avatar, dogMap[data.dog], 88);
  }
  ...
  const oldSelectOption = window.selectOption;
  if (typeof oldSelectOption === 'function') {
    window.selectOption = function(i, el){
      oldSelectOption(i, el);
      setTimeout(function(){
